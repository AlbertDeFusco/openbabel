...
int main(int argc, char **argv)
{
	...
	if (argc != 3) {
		cerr << " Usage: " << program_name << " <input file> <atomic number>\n";
		exit(-1);
	}
	OBMol mol;
	...
	for (;;) {
		...
		for(itr=mols.begin();itr!=mols.end();++itr) {
			...
			FOR_ATOMS_OF_MOL(atom,*itr)
			{
				...
				double thismass=atom->GetAtomicMass();
				totalmass=totalmass+thismass;

				numi++;
				Nx=Nx + atom->GetX()*thismass;
				Ny=Ny + atom->GetY()*thismass;
				Nz=Nz + atom->GetZ()*thismass;
			}
			Nx=Nx/totalmass;
			Ny=Ny/totalmass;
			Nz=Nz/totalmass;
			//cerr << "COM:" << " " << Nx << " " << Ny << " " << Nz << endl;
			...
			//}
			if(numi==0) {
				cerr << "COM solute not found." << endl;
				exit(-1);
			}
		}
